...
(* gakusei_t list は
    - []                空リスト、あるいは
    - first :: rest )   最初の1要素が firstで残りのリストがrest
                        (first は gakusei_t 型
                        rest が自己参照のケース 
  という形。 *)
...
| ({ namae = n; tensuu= t; seiseki = s} as x) :: rest ->
  if t > gakusei_tensuu (max rest) then x else max rest

(* テスト *)
let test1 = max lst2 = {namae = "asai"; tensuu = 70; seiseki = "B"}
let test2 = max lst3 = {namae = "kaneko"; tensuu = 85; seiseki = "A"}
let test3 = max lst4 = {namae = "kaneko"; tensuu = 85; seiseki = "A"}